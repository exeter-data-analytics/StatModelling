...
Both of these cases are **exact** tests for linear regression with Gaussian errors (but for mixed models these become approximate). Let's have a look at a simple example using the fruitflies data from earlier practicals.
...
> **Note**: I can only use the `anova()` function in this way because the data are **balanced** (or at least the model formulation above with no nesting and no pseudoreplication ends up treating the data s if they were balanced). In general you have to be very careful to use `anova()` in this way.
...
```{r}
## predict yield from model
newdata <- expand.grid(
    density = levels(splityield$density),
    irrigation = levels(splityield$irrigation),
    fertilizer = levels(splityield$fertilizer)
)
newdata$yield <- predict(split_lmer, newdata, re.form = NA)

## generate model matrix for predictions
mm <- model.matrix(terms(split_lmer), newdata)

## generate error terms
pvar1 <- diag(mm %*% tcrossprod(vcov(split_lmer), mm))

## use Gaussian approximation
cmult <- 1.96 
newdata <- cbind(
    newdata,
    lci = newdata$yield - cmult * sqrt(pvar1),
    uci = newdata$yield + cmult * sqrt(pvar1)
)
```
...
ggplot(newdata, 
    aes(x = fertilizer, colour = density, shape = density)) + 
...
boot1 <- bootMer(split_lmer, predFun, nsim = 250, type = "parametric", re.form = NA)
newdata <- cbind(newdata, sumBoot(boot1))
...
```{r}
newdata <- expand.grid(
    block = levels(splityield$block),
    density = levels(splityield$density),
    irrigation = levels(splityield$irrigation),
    fertilizer = levels(splityield$fertilizer)
)
predFun <- function(mod) {
  predict(mod, newdata = newdata, re.form = NULL)
}
boot2 <- bootMer(split_lmer, predFun, nsim = 250, type = "parametric", re.form = NULL)
split_boot2 <- cbind(newdata, sumBoot(boot2))
```

## Bayesian modelling {#secbayes}

Many of the challenges associated with **mixed effects** models go away if you move your inference into a **Bayesian** framework. Thats not to say that other challenges do not arise in their place, mainly in terms of variable selection, however, in general I would recommend using a Bayesian framework for complex models with hierarchical structures, particularly spatio-temporal modelling. 
