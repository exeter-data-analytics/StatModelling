...
Take a look at the data and the help file for abrasion (i.e. `?abrasion`). Fit an appropriate linear mixed model to these data and assess whether there is any statistical evidence for differences in wear rates between the different materials, with material `B` wearing the fastest, followed by `D`, `C` and `A`.
...
This suggests that there are highly statistically significant differences in wear between *at least some* of the materials.
```
...
4. Use a likelihood ratio test to assess the impact of `freshener` on the model fit.
...
``{r, eval = F}
drunk <- readRDS("drunk.rds")
alc <- aggregate(alcohol ~ student + freshener, data = drunk, mean)
alc_lm <- lm(alcohol ~ freshener, data = alc)
drop1(alc_lm, test = "Chisq")
``
