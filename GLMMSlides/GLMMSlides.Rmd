...
## Aside: NHST
\small
For mixed models things get trickier again, and there is no consensus---see [GLMM FAQ](https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html) for more discussion. A nice description of the types of approaches we can use in different cases can be found at:
...
Mixed models fitted using **REML**.
...
Use `update` but with `REML = F` argument. 

This converts to **unrestricted maximum likelihood** (so it's a **biased** approximation, but usually a good one).

Switch back to REML fit to perform inference. See **Section 4.2.1.1** of the notes for more details.
...
We ideally want to build a model that **accounts** for the variation due to the **pseudoreplicates**, but models the effect we're interested in: here the effect of **breath freshener** on **alcohol content**.
...
```{r hg1, results = "hide", warning = F, message = F, size = "tiny"}
