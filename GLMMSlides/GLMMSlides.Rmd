...
\includegraphics[width=0.5\textwidth]{images/withnail.png}
...
<!-- \includegraphics[width = 0.5\textwidth]{images/vyvyan.gif} -->
VYVYAN
...
\includegraphics[width = 0.5\textwidth]{images/drunkdesign.png}

## Results

```{r drunkres, echo = F, fig.width = 8}
...
```{r drunk, echo = -1}
drunk <- read.csv("./data/blood_alcohol.csv", header = T)
drunk.lm <- lm(alcohol ~ freshener, data = drunk)
anova(drunk.lm)
```
...
* **Clue**: residual df (`r anova(drunk.lm)$Df[2]`) is much bigger than the number of ***experimental units*** (students: `r length(levels(drunk$student))` here).
...
alc
alc.lm <- lm(alcohol ~ freshener, data = alc)
anova(alc.lm)
```
...
2. **Nested analysis**:
    * Uses all data.
    * **Partitions** the residual deviance.
    * Only works with `aov` command in R (which also allows **blocking** and **split-plot** experimental designs).

## Nested designs in R

`Error` section tells R how residuals should be structured:  
i.e. multiple samples **within** students.

Here only **freshener** effect is of interest.

```{r drunkaov}
drunk.aov <- aov(alcohol ~ freshener 
                    + Error(student/sample), data = drunk)
summary(drunk.aov)
```
...
> What is the probability of testing positive given that you've used breath freshener relative to drinking alcohol?

Beware of **proxy** measurements.

## Blocked experimental design

Recognise natural structuring among experimental units.

Source of error (e.g. top/bottom of field, make of car, student identity).
...
\includegraphics[width = 0.5\textwidth]{images/blockdesign.png}

## Why use blocks?

Here **TREATMENT** is of interest, but multiple treatments within each block.
...
Pretend cabinet is a **fixed effect**.

Test **full** model (cabinet, media and interaction):
...
Model summary is a bunch of `NA`s and simplification doesn't work. Why?

## Analyse it properly

Full model is **saturated** (no residual df).

Assume **no interaction** between `cabinet` and `media`.

Test effect of `media` against deviance explained by `cabinet:media`.

In general, test **main** effect against its **interaction with block**.
...
```{r bacaov}
bac.aov <- aov(growth ~ media + Error(cabinet/media), data = bac)
summary(bac.aov)
```
