...
If assumptions 1. to 3. are violated we can *transform* our response variable
...
Count data are ubiquitous in the life sciences (e.g number of parasites per microliter of blood,
...
\end{aligned}
$$
The mean function is **unconstrained**, i.e the value of $\beta_0 + \beta_1X$ can range from $-\infty$ to $+\infty$. If we want to model count data we therefore want to **constraint** this mean to be positive. Mathematically we can do this by taking the **logarithm** of the mean (it is by no coincidence that log transforms are very popular to normalise response variables). We then assume our count data to be Poisson distributed (a discrete, non-negative distribution), to obtain our Poisson regression model (to be consistent with the statistics literature we will rename $\mu$ to $\lambda$):
...
> Do voters with higher incomes prefer conservative candidates?
...
We have a dataset of chemical analysis of wines grown in the same region in Italy but derived from three different cultivars. 
