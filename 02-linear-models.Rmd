...
The object returned by the `lm()` contains further information that we can display using the `summary()` function.
...
* **Coefficients**
    ...
    If the standard error is comparable or greater than the actual parameter estimate then that parameter should not
    be trusted. We can also show to the confidence intervals for model parameters to highlight the uncertainty aroung
    the parameter estimate using the `confint()` function:
...
df <- data.frame(weight=weight, heightParents=heightParents, height=height) # store in a data frame
...
`response ~ explanatory1 + explanatory2`, which in our case this will be `height ~ weight + heightParents`.
