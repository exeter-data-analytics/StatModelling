...
This is of course nonsense, but I'm highlighting it as a reminder that data-driven models only make sense close to the observed data, in general
...
[I disagree: extrapolating is not bad. Sometimes it is incredibly useful and sometimes it is the principal reason **why** we fit the model. It is however, dangerous. So It's not that it only makes sense where we have observed data, it's rather that we can only assess the assumptions of the model over the range of the observed data. When extrapolating we have to assume that these relationships hold beyond the range of the data, which may or may not be reasonable (hence why weather forecast over short-time periods are OK, but climate forecasts are much more uncertain). Hence the model is an approximation, that in this case is not sensible where $x = 0$, but we don't care about when $x = 0$, so it's probably OK for our purposes. If we did care then we'd have to change the model so that we assume some behaviour at other points in the range.]
>>>>>>> 10bc3b1b4024c8607363a383c16a418711f4ef84
...
for $\beta_1 \times \mathrm{weight}$ to return cm, $\beta_1$ needs to be cm/kg (i.e you have $\frac{\text{cm}}{\text{kg}}\text{kg}=\text{cm}$)

[This gradient / units argument is a bit confusing. I'd remove the **note**, and just say that R calls the coefficient by the name of the variable for ease of understanding.]
...
[I just wonder here whether you should say that this performs a hypothesis test for the null hypothesis that the coefficient is zero. Whether it's statistically significant or not depends on what significance level you choose. This will always chuck out a p-value regardless of your pre-defined level of significance. (i.e. separate the test result from the interpretation of the test result).]
...
<<<<<<< HEAD
The F-statistic tests whether the amount of variation explained by the regression is *statistically significant* compared to the **null** model ($M_0$ - in this case the **null** model is the same as just taking the mean of the data i.e the gradient is zero).
=======
The F-statistic can be used to assess [SEE COMMENT ABOVE] whether the amount of variation explained by the regression is *statistically significant* compared to the **null** model ($M_0$ - in this case the **null** model is the same as just taking the mean of the data).
>>>>>>> 10bc3b1b4024c8607363a383c16a418711f4ef84
...
[No data link here, so I guess you haven't pushed to repo.]
...
[Could you add here a first task to plot the data before fitting the linear model.]
